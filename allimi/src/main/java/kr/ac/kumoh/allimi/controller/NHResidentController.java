...
public class NHResidentController {
    ...
    public ResponseEntity addNHResident(@Valid @RequestBody NHResidentDTO dto) { // user_id, facility_id, resident_name, birth, user_role, health_info;
        //입소자 중복 추가 방지용
    ...
    public ResponseEntity deleteResident(@RequestBody Map<String, Long> resident) throws Exception { //nhresident_id
        ...
        if (userId == null || residentId == null)
            throw new NHResidentException("NHResidentController 입소자 삭제: user_id 또는 nhresident_id가 null. 잘못된 입력");
    ...
    public ResponseEntity manageNHResidentList(@RequestBody Map<String, Long> resident) throws Exception {
        ...
        if (user_id == null)
            throw new UserException("NHResidentController 요양보호사가 관리하는 입소자 목록: user_id or facility_id가 null로 들어옴");

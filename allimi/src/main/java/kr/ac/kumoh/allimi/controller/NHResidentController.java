...
public class NHResidentController {
    ...
    public ResponseEntity deleteResident(@RequestBody Map<String, Long> resident) throws Exception { //nhresident_id
        ...
        if (userId == null)
            throw new UserException("NHResidentController 입소자 삭제: user_id가 null. 잘못된 입력");

        Long residentId = resident.get("nhresident_id");
        if (residentId == null)
            throw new NHResidentException("NHResidentController 입소자 삭제: nhresident_id가 null. 잘못된 입력");
    ...
    public ResponseEntity v2UsersNHResidentList(@PathVariable("user_id") Long userId) throws Exception { //user_id
        if (userId == null)
            throw new UserException("NHResidentController 입소자 리스트 출력: user_id가 null. 잘못된 입력");
    ...
    public ResponseEntity nhresidentList(@PathVariable("nhr_id") Long nhrId) throws Exception { //nhr_id
        if (nhrId == null)
            throw new NHResidentException("NHResidentController 입소자 상세정보 출력: nhr_id가 null. 잘못된 입력");
    ...
    public ResponseEntity protectorList(@PathVariable("facility_id") Long facilityId) throws Exception {
        if (facilityId == null)
            throw new FacilityException("NHResidentController 시설의 모든 보호자 출력: facility_id가 null. 사용자의 잘못된 입력");
    ...
    public ResponseEntity nhResidentWithFacilityList(@PathVariable Long user_id) throws Exception {
        if (user_id == null)
            throw new UserException("NHResidentController 사용자가 등록한 요양원 목록: user_id가 null로 들어옴");
    ...
    public ResponseEntity manageNHResidentList(@Valid @RequestBody UserFacilityDTO userFacilityDTO) throws Exception {
        List<NHResidentResponse> responseList = nhResidentService.workerList(userFacilityDTO.getUser_id(), userFacilityDTO.getFacility_id());
